...
class RestaurantRecommender:
    def __init__(self):
        ### Initialize the Class, train necessary model.
    ...
    def find_restaurant(self):
        ...
        # Handle 'any' inputs and find matching restaurants
        if self.area != "any":
            restaurant_match_df = restaurant_match_df[
                restaurant_match_df["area"].str.lower() == self.area.lower()
            ]
        if self.food_type != "any":
            restaurant_match_df = restaurant_match_df[
                restaurant_match_df["food"].str.lower() == self.food_type.lower()
            ]
        if self.price_range != "any":
            restaurant_match_df = restaurant_match_df[
                restaurant_match_df["pricerange"].str.lower()
                == self.price_range.lower()
            ]
        ...
        # Find closest matches, where 2 out of 3 preferences are satisfied
        closest_match_df = self.restaurant_df.copy()
        if self.area != "any":
            closest_match_df = closest_match_df[
                closest_match_df["area"].str.lower() == self.area.lower()
            ]
        if self.food_type != "any":
            closest_match_df = closest_match_df[
                closest_match_df["food"].str.lower() == self.food_type.lower()
            ]
        if not closest_match_df.empty:
            return closest_match_df, "price_range"
        if self.price_range != "any":
            closest_match_df = closest_match_df[
                closest_match_df["pricerange"].str.lower() == self.price_range.lower()
            ]
        if self.food_type != "any":
            closest_match_df = closest_match_df[
                closest_match_df["food"].str.lower() == self.food_type.lower()
        ...
        if self.price_range != "any":
            closest_match_df = closest_match_df[
                closest_match_df["pricerange"].str.lower() == self.food_type.lower()
            ]
        if not closest_match_df.empty:
            return closest_match_df, "area"
    ...
    def filter_restaurants(self, restaurants):
        ...
        for r in restaurants.itertuples():
            inferred = utils.infer_properties(
                [r.pricerange, r.food_quality, r.crowdedness, r.length_of_stay]
            )
            if inferred.get(self.special_feature) is True:
                filtered_restaurants = pd.concat(
                    [
                        filtered_restaurants,
                        restaurants[restaurants.restaurantname == r.restaurantname],
                    ]
                )
    ...
    def give_recommendation(self):
        ### Output of Restaurant Recommendation with a relevant comment
    ...
    def extract_preferences(self, utterance, current_state):
        ...
        # Workaround for handling test-cases with 'world' cuisine and 'asian'
    ...
    def get_next_state(
        ...
        if current_state == State.WELCOME:
            ...
            self.matching_restaurants = self.filter_restaurants(
                self.matching_restaurants
            )
    ...
    def run(self):
        ### Dialog Machine core function that runs as long as the conversation is going and handles inputs and outputs for all states
        last_state = None
        current_state = State.WELCOME

        dialog_act, user_utterance = self.collect_input(
        ...
        while current_state != State.EXIT:
            ...
            if next_state == State.WELCOME:
                if last_state is State.WELCOME:
                    dialog_act, user_utterance = self.collect_input(
                        "Great to have you here! How can I help?"
                    )
                else:
                    dialog_act, user_utterance = self.collect_input(
                ...
                if last_state is State.ASK_AREA:
                    dialog_act, user_utterance = self.collect_input(
                        "What area would you like to eat in? Choose from centre, east, west, south and north."
                    )
                else:
                    dialog_act, user_utterance = self.collect_input(
                        "In which area are you looking for a restaurant?"
                    )
                ...
                if last_state is State.ASK_FOOD:
                    dialog_act, user_utterance = self.collect_input(
                        "We don't seem to have restaurants of that type, try a different one?"
                    )
                else:
                    dialog_act, user_utterance = self.collect_input(
                ...
                if last_state is State.ASK_PRICE:
                    dialog_act, user_utterance = self.collect_input(
                        "What prices do you prefer? You can choose from cheap, moderate and expensive."
                    )
                else:
                    dialog_act, user_utterance = self.collect_input(
                ...
                if last_state is State.ASK_REQUIREMENTS:
                    dialog_act, user_utterance = self.collect_input(
                        "Do you have special wishes? You can choose from touristic, romantic, assigned seats or children friendly."
                    )
                else:
                    dialog_act, user_utterance = self.collect_input(
                        "Do you have any additional requirements for the restaurant?"
                    )
                ...
                if dialog_act == "reqalts":
                    if self.matching_restaurants.empty is True:
                        self.output_utterance(
                            "Unfortunately there are no other restaurants matching your criteria."
                        )
                        dialog_act, user_utterance = self.collect_input(
                ...
                self.output_utterance(
                    f"The phone number of restaurant {string.capwords(self.current_recommendation.restaurantname) } is {self.current_recommendation.phone}"
                )

            elif next_state == State.PROVIDE_ADDRESS:
                # Provide restaurant address
                self.output_utterance(
                    f"The address of restaurant {string.capwords(self.current_recommendation.restaurantname) } is {self.current_recommendation.addr}"
                )

            elif next_state == State.PROVIDE_POSTCODE:
                # Provide restaurant postcode
                self.output_utterance(
                    f"The postcode of restaurant {string.capwords(self.current_recommendation.restaurantname) } is {self.current_recommendation.postcode}"
                )
                ...
                # Say goodbye
                self.output_utterance(
                    "Hope you enjoyed using our system, see you next time!"
                )
                exit
